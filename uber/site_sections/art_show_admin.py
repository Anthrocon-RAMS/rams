...
class Root:
    ...
    def piece_labels_pdf(self, session, id, **params):
        ...
        app = session.art_show_application(id)
        pieces = sorted(app.art_show_pieces, key=lambda p: (p.gallery_label, p.piece_id))

        # Avery 5160: 30 labels per sheet (3 cols x 10 rows)
        # Label: 2.625" wide x 1" tall = 189 x 72 pt
        # Left margin: 0.1875" = 13.5 pt, top margin: 0.5" = 36 pt
        # Column gap: 0.125" = 9 pt, row gap: 0 pt
        LABEL_W = 189.0
        LABEL_H = 72.0
        LEFT_MARGIN = 13.5
        TOP_MARGIN = 36.0
